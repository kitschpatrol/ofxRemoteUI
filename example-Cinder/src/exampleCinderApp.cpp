...
class exampleCinderApp : public App {
public:
...
void exampleCinderApp::prepareSettings(Settings *settings) {
...
void exampleCinderApp::setup() {
	...
	mBackgroundColor = ColorA8u(1.0, 0.0, 0.5, 1.0);
	mCurrentShape = ShapeOptions::SQUARE;
	mRotationSpeed = 0.01;
	...
	RUI_SET_SIGNAL(&exampleCinderApp::serverCallback);
...
void exampleCinderApp::draw() {
	...
	switch (mCurrentShape) {
		case SQUARE:
			...
			break;
		case CIRCLE:
			gl::drawSolidCircle(vec2(0), 0.5, 36);
			break;
		case TRIANGLE:
...
void exampleCinderApp::serverCallback(RemoteUIServerCallBackArg arg) {
	switch (arg.action) {
...
CINDER_APP(exampleCinderApp, RendererGl(RendererGl::Options().msaa(4)), exampleCinderApp::prepareSettings)
