...
bool ofxRemoteUIClient::setup(string address, int port_) {
	...
	if (verbose_)
		RLOG_NOTICE << "connecting to " << address;
	try {
...
void ofxRemoteUIClient::disconnect() {
	...
	if (readyToSend) {
		if (verbose_)
			RLOG_NOTICE << "disconnect()";
		sendCIAO();
		readyToSend = false;
		presetNames.clear();
	} else {
		if (verbose_)
			RLOG_NOTICE << "can't disconnect(); we arent connected!";
	}
}
...
void ofxRemoteUIClient::connect() {
	...
	if (!readyToSend) {
		...
		if (verbose_)
			RLOG_NOTICE << "can't connect() now, we are already connected!";
	}
}
		...
		while (oscReceiver.hasWaitingMessages()) { // check for waiting messages from server
			...
			switch (dm.action) {
				...
				case REQUEST_ACTION: // server closed the REQU, so we should have all the params
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says REQUEST_ACTION!";
					if (callBack != NULL) {
				...
				case SEND_PARAM_ACTION: { // server is sending us an updated val
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says SEND_PARAM_ACTION!";
				...
				case PRESET_LIST_ACTION: // server sends us the list of current presets
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says PRESET_LIST_ACTION!";
				...
				case SET_PRESET_ACTION: // server confirms that it has set the preset, request a full update
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says SET_PRESET_ACTION!";
					requestCompleteUpdate();
					if (callBack != NULL) {
				...
				case SAVE_PRESET_ACTION: { // server confirms that it has save the preset,
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says SAVE_PRESET_ACTION!";
					vector<string> a;
				...
				case DELETE_PRESET_ACTION: { // server confirms that it has deleted preset
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says DELETE_PRESET_ACTION!";
					vector<string> a;
					sendPREL(a); // request preset list to server, send empty vector
					if (callBack != NULL) {
				...
				case RESET_TO_XML_ACTION: { // server confrims
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says RESET_TO_XML_ACTION!";
					requestCompleteUpdate();
					if (callBack != NULL) {
				...
				case RESET_TO_DEFAULTS_ACTION: { // server confrims
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says RESET_TO_DEFAULTS_ACTION!";
					requestCompleteUpdate();
					if (callBack != NULL) {
				...
				case SAVE_CURRENT_STATE_ACTION: // server confirms that it has saved
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says SAVE_CURRENT_STATE_ACTION!";
					if (callBack != NULL) {
				...
				case SET_GROUP_PRESET_ACTION: // server confirms that it has set the group preset, request a full update
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says SET_GROUP_PRESET_ACTION!";
					requestCompleteUpdate();
					if (callBack != NULL) {
				...
				case SAVE_GROUP_PRESET_ACTION: { // server confirms that it has saved the group preset,
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says SAVE_GROUP_PRESET_ACTION!";
					vector<string> a;
					sendPREL(a); // request preset list to server, send empty vector
				...
				case DELETE_GROUP_PRESET_ACTION: { // server confirms that it has deleted the group preset
					if (verbose_)
						RLOG_VERBOSE << m.getRemoteIp() << " says DELETE_GROUP_PRESET_ACTION!";
					vector<string> a;
					sendPREL(a); // request preset list to server, send empty vector
					if (callBack != NULL) {
				...
				default:
					RLOG_ERROR << "update >> UNKNOWN ACTION!!" << endl;
					break;
			}
		}
	}
