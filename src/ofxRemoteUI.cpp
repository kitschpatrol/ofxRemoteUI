...
DecodedMessage ofxRemoteUI::decode(ofxOscMessage m) {
	...
	if (msgAddress.length() >= 3) {
		if (action == "HELO")
			dm.action = HELO_ACTION;
		else if (action == "REQU")
			dm.action = REQUEST_ACTION;
		else if (action == "SEND")
			dm.action = SEND_PARAM_ACTION;
		else if (action == "CIAO")
			dm.action = CIAO_ACTION;
		else if (action == "TEST")
			dm.action = TEST_ACTION;
		else if (action == "PREL")
			dm.action = PRESET_LIST_ACTION;
		else if (action == "DELP")
			dm.action = DELETE_PRESET_ACTION;
		else if (action == "SAVP")
			dm.action = SAVE_PRESET_ACTION;
		else if (action == "SETP")
			dm.action = SET_PRESET_ACTION;
		else if (action == "RESX")
			dm.action = RESET_TO_XML_ACTION;
		else if (action == "RESD")
			dm.action = RESET_TO_DEFAULTS_ACTION;
		else if (action == "SAVE")
			dm.action = SAVE_CURRENT_STATE_ACTION;
		else if (action == "MISP")
			dm.action = GET_MISSING_PARAMS_IN_PRESET;
		// groups (note lower case p, l)
		else if (action == "DELp")
			dm.action = DELETE_GROUP_PRESET_ACTION;
		else if (action == "SAVp")
			dm.action = SAVE_GROUP_PRESET_ACTION;
		else if (action == "SETp")
			dm.action = SET_GROUP_PRESET_ACTION;
		// log
		else if (action == "LOG_")
	...
	if (msgAddress.length() >= 8) {
		...
		if (arg1 == "FLT")
			...
			dm.argument = ENUM_ARG;
		else if (arg1 == "COL")
			dm.argument = COLOR_ARG;
		else if (arg1 == "SPA")
			dm.argument = SPACER_ARG;
	} else {
		...
		for (ifa = myaddrs; ifa != NULL; ifa = ifa->ifa_next) {
			...
			if (ifa->ifa_addr->sa_family == AF_INET) {
				...
				if (inet_ntop(ifa->ifa_addr->sa_family, (void *)&(s4->sin_addr), buf, sizeof(buf)) == NULL) {
					...
					if (verbose_)
						RLOG_VERBOSE << "found interface: " << interface;
...
void ofxRemoteUI::updateParamFromDecodedMessage(ofxOscMessage m, DecodedMessage dm) {
	...
	switch (dm.argument) {
		case FLT_ARG:
			...
			arg++;
			if (m.getNumArgs() > 1) {
				p.minFloat = m.getArgAsFloat(arg);
				arg++;
		...
		case COLOR_ARG:
			...
			arg++;
			p.blueVal = (int)m.getArgAsInt32(arg);
			arg++;
		...
		case NULL_ARG:
			RLOG_ERROR << "updateParamFromDecodedMessage NULL type!";
			break;
		default:
			RLOG_ERROR << "updateParamFromDecodedMessage unknown type!";
			break;
	}

	if (m.getNumArgs() > 1) {
		p.r = m.getArgAsInt32(arg);
		arg++;
		p.g = m.getArgAsInt32(arg);
		arg++;
		p.b = m.getArgAsInt32(arg);
		arg++;
		p.a = m.getArgAsInt32(arg);
		arg++;
		p.group = m.getArgAsString(arg);
		arg++;
	}
	...
	switch (p.type) {
		case REMOTEUI_PARAM_FLOAT: // GSFDONE
			if (p.isUsingGetterSetter()) {
				...
				return (*p.floatValAddr != p.floatVal);
			}
			return false;
		...
		if (params.find(name) != params.end()) {
			...
			switch (param.type) {
				...
				case REMOTEUI_PARAM_COLOR: { // GSCDONE
					...
					param.redVal = atoi(token.c_str());
					std::getline(ss, token, ' ');
					param.greenVal = atoi(token.c_str());
					std::getline(ss, token, ' ');
					param.blueVal = atoi(token.c_str());
					std::getline(ss, token, ' ');
...
void ofxRemoteUI::sendMISP(vector<string> missingParamsInPreset) {
	if (missingParamsInPreset.size() == 0)
		return; // do nothing if no params are missing
	if (verbose_)
		RLOG_VERBOSE << "sendMISP()";
	ofxOscMessage m;
	m.setAddress("/MISP");
