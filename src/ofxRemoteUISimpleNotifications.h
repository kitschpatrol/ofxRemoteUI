...
public:
	...
	void draw(float x, float y) {
		...
		for (int i = 0; i < notifications.size(); i++) {
			...
			float hh = drawStringWithBox("ofxRemoteUIServer: " + notifications[i].msg, x, yy, ofColor(0, 255 * a), NOTIFICATION_COLOR);
			yy -= hh;
		}
		...
		for (it_type it = paramNotifications.begin(); it != paramNotifications.end(); it++) {
			...
			float hh = drawStringWithBox(total, x, yy, bgColor, ofColor::black);
		...
		while (it2 != paramWatchOrder.end()) {
			...
			float hh = drawStringWithBox("[" + it2->second + "] " + paramWatch[name].value, x, yy, ofColor::black, paramWatch[name].color);
			yy -= hh;
			++it2;
		}
	};
