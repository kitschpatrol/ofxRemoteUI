...
void ofxRemoteUIServer::setDrawsNotificationsAutomaticallly(bool draw) {
	drawNotificationsAutomatically = draw;
}
...
ofxRemoteUIServer::ofxRemoteUIServer() {
	...
	drawNotificationsAutomatically = true;
	showUI = false;
	loadedFromXML = false;
...
void ofxRemoteUIServer::removeParamFromDB(string paramName) {
	...
	if (it != params.end()) {
		...
		if (verbose_)
			RLOG_WARNING << "removing Param '" << paramName << "' from DB!";
...
void ofxRemoteUIServer::saveParamToXmlSettings(const RemoteUIParam &t, string key, ofxXmlSettings &s, XmlCounter &c) {
	...
	switch (t.type) {
		...
		case REMOTEUI_PARAM_COLOR: { // GSCDONE
			...
			if (verbose_)
				RLOG_NOTICE << "saving '" << key << "' (" << r << " " << g << " " << b << " " << a << ") to XML";
...
void ofxRemoteUIServer::saveParamToXmlSettings(const RemoteUIParam &t, string key, ofXml &s, int index, bool active) {
	...
	switch (t.type) {
		...
		case REMOTEUI_PARAM_SPACER:
			if (verbose_)
				RLOG_NOTICE << "save spacer '" << key << "' to XML";
			s.addChild("P");
			s.setTo(path);
...
void ofxRemoteUIServer::saveToXMLv2(string fileName, string groupName) {
	...
	if (!savingGroupOnly) { // group presets dont save disabled stuff
		...
		if (orderedKeys_removed.size()) {
			Poco::XML::Element *el = s.getPocoElement();
			Poco::XML::Comment *ce =
					s.getPocoDocument()->createComment(Poco::XML::toXMLString("                                                                                 "));
				...
				switch (type[0]) {
					...
					case 'g': { // group
						if (!isAParamWeKnowOf) {
							...
							if (verbose_)
								RLOG_NOTICE << "skipping GROUP '" << paramName << "' from XML";
						}
					} break;
...
void ofxRemoteUIServer::saveSettingsBackup() {
	...
	if (autoBackups) {
		...
		f.close();
		if (verbose_)
			RLOG_NOTICE << "saving a backup of the current " << getFinalPath(OFXREMOTEUI_SETTINGS_FILENAME) << " in "
									<< getFinalPath(OFXREMOTEUI_SETTINGS_BACKUP_FOLDER);
	}
#endif
}
...
// still get ui access despite being disabled
	...
	// Exit
	ci::app::App::get()->getSignalCleanup().connect([this]() { prepareForExit(); });
	...
	if (showUI) {
		switch (e.key) { // you can save current config from tab screen by pressing s
			...
			case 'S': {
				...
				string presetName = ofSystemTextBoxDialog(groupIsSelected ? "Create a New Group Preset For " + groupName : "Create a New Global Preset", "");
				if (presetName.size()) {
					...
					if (groupIsSelected) {
						...
						if (verbose_)
							RLOG_NOTICE << "saving NEW preset: " << presetName;
			...
			case OF_KEY_RETURN:
				if (selectedItem == -1 && selectedPreset >= 0 && presetsCached.size()) { // global presets
					...
					uiAlpha = 0;
					if (verbose_)
						RLOG_NOTICE << "setting preset: " << lastChosenPreset;
					RemoteUIServerCallBackArg cbArg;
				...
				if (selectedItem >= 0) { // selection on params list
					...
					if (p.type == REMOTEUI_PARAM_SPACER && groupPresetsCached[p.group].size() > 0) {
						...
						uiAlpha = 0;
						if (verbose_)
							RLOG_NOTICE << "setting preset: " << presetName;
						RemoteUIServerCallBackArg cbArg;
			...
			case OF_KEY_UP: {
				...
				if (selectedItem < -1)
					selectedItem = orderedKeys.size() - 1;
				if (selectedItem >= orderedKeys.size())
					selectedItem = -1; // presets menu >> selectedItem = -1, on top of all
				selectedGroupPreset = 0;
			} break;

			case ',':
				xOffsetTarget += (uiColumnWidth);
				xOffsetTarget = ofClamp(xOffsetTarget, -FLT_MAX, 2 * uiColumnWidth);
				break;
			case '.':
				xOffsetTarget -= (uiColumnWidth);
				xOffsetTarget = ofClamp(xOffsetTarget, -FLT_MAX, 2 * uiColumnWidth);
				...
				if (selectedItem >= 0) { // params
					...
					if (p.type != REMOTEUI_PARAM_SPACER) {
						...
						if (numGroupPresets > 0) {
							...
							if (selectedGroupPreset < 0)
								selectedGroupPreset = numGroupPresets - 1;
							if (selectedGroupPreset > numGroupPresets - 1)
								selectedGroupPreset = 0;
						}
					}
				} else { // presets!
...
void ofxRemoteUIServer::threadedFunction() {
	...
	if (verbose_)
		RLOG_NOTICE << "threadedFunction() ending";
}
...
void ofxRemoteUIServer::draw(int x, int y) {
	...
	if (showUI) {
		...
		if (uiAlpha > 0.99 || showUIduringEdits) {
			...
			ofSetColor(255);
			drawString("ofxRemoteUI built in client. " + string(enabled ? ("Server reachable at " + computerIP + ":" + ofToString(port)) + "." : "Sever Disabled.") +
								 ...
								 padding, screenH / uiScale - bottomBarHeight + 20);
		}
		...
		ofColor textBlinkC;
		if (frameNum % 6 < 4)
			textBlinkC = ofColor(0, 0);
		else
			textBlinkC = ofColor(255, 0, 0);
		...
		if (uiAlpha > 0.99 || showUIduringEdits) {
			...
			for (int i = 0; i < orderedKeys.size(); i++) {
				...
				switch (p.type) {
					...
					default:
						printf("weird RemoteUIParam at draw()!\n");
						break;
				}
				if (!linesInited) {
		...
		if (uiAlpha < 1.0 && !showUIduringEdits) {
			...
			for (ang = 0; ang < lim; ang += step) {
				m.addVertex(origin + ofVec2f(r * cosf((-90.0f + ang) * DEG_TO_RAD), r * sinf((-90.0f + ang) * DEG_TO_RAD)));
			}
			float lastBit = lim - ang;
			m.addVertex(origin + ofVec2f(r * cosf((-90.0f + ang + lastBit) * DEG_TO_RAD), r * sinf((-90.0f + ang + lastBit) * DEG_TO_RAD)));
			ofSetColor(128);
			m.draw();
		}
	...
	if (needsToDrawNotification) {
		if (drawNotificationsAutomatically) {
	...
	while (oscReceiver.hasWaitingMessages()) { // check for waiting messages from client
		...
		switch (dm.action) {
...
#endif
				if (verbose_)
					RLOG_VERBOSE << m.getRemoteIp() << " says HELLO!";

				break;

			case REQUEST_ACTION: { // send all params to client
				if (verbose_)
					RLOG_VERBOSE << m.getRemoteIp() << " sends REQU!";
				pushParamsToClient();
			} break;

			case SEND_PARAM_ACTION: { // client is sending us an updated val
				if (verbose_)
					RLOG_VERBOSE << m.getRemoteIp() << " sends SEND!";
			...
			case CIAO_ACTION: {
				...
				readyToSend = false;
				if (verbose_)
					RLOG_VERBOSE << m.getRemoteIp() << " says CIAO!";
			} break;
			...
			case DELETE_PRESET_ACTION: {
				...
				ofNotifyEvent(clientAction, cbArg, this);
				if (verbose_)
					RLOG_NOTICE << "DELETE preset: " << presetName;
#endif
#ifdef CINDER_AVAILABLE
				cbSignal.emit(cbArg);
#endif
			...
			case SAVE_CURRENT_STATE_ACTION: {
				...
				sendSAVE(true);
				if (verbose_)
					RLOG_NOTICE << "SAVE CURRENT PARAMS TO DEFAULT XML: ";
			} break;
...
#endif
				if (verbose_)
					RLOG_NOTICE << "setting preset group: " << groupName << "/" << presetName;
			} break;
...
void ofxRemoteUIServer::deletePreset(string name, string group) {
	...
	if (group != "")
		file = getFinalPath(OFXREMOTEUI_PRESET_DIR) + "/" + group + "/" + name + ".xml";
	remove(file.c_str());
#endif
}
...
void ofxRemoteUIServer::shareParam(string paramName, std::function<float()> getter, std::function<void(float)> setter, float min, float max, ofColor c) {
	...
	p.maxFloat = max;
	p.minFloat = min;
	p.floatVal = ofClamp(getter(), min, max);
	p.floatGetter = getter;
	p.floatSetter = setter;
...
void ofxRemoteUIServer::shareParam(string paramName, unsigned char *param, ofColor bgColor, int nothingUseful) {
	...
	addParamToDB(p, paramName);
	if (verbose_)
		RLOG_NOTICE << "Sharing Color Param '" << paramName << "'";
}
